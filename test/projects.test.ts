...
describe("Projects MCP Tests", async () => {
  ...
  it("should list all projects in a workspace", async function() {
    // Skip test if TEST_WORKSPACE_ID is not defined
    if (!TEST_WORKSPACE_ID) {
      console.log("Skipping projects test: TEST_WORKSPACE_ID not defined");
      this.skip();
      return;
    }
    ...
    try {
      ...
      // If there are projects, verify they have the expected properties
      if (projects.length > 0) {
        const firstProject = projects[0];
        assert(typeof firstProject.id === "string");
        assert(typeof firstProject.name === "string");
        // clientName might be null depending on the project setup
        assert("clientName" in firstProject);
      }
    } catch (error) {
